...
# COMM: correct typo & fix issue which LZMA compressed info inside split_result[6]
#       did not count the LZMA header&tail part size inside. it only caculate for
...
$LZMA = catfile(dirname(__FILE__), "lzma_asr$EXE_SUFFIX");
...
while(1){
	...
	# step2: compressed the OUTBIN_TMP & get the lzma info
	#
	$lowercase_tmp_lzma_file=lc("$mark\_$TMP_LZMA"); # set to lowercase to avoid case-change under linux env , with WINE lzma.exe invoke
	$temp_index=index($IMG_GENERAL_INFO,"8M");
	if ($temp_index == -1){
		$temp_index=index($IMG_GENERAL_INFO,"4M");
		if ($temp_index == -1){
			qx("$LZMA" ds 19 e $mark\_$TMP $lowercase_tmp_lzma_file 2>&1 );
		}else{
			print sprintf ("|%s|\n","LteOnly 4M RAM");
			qx("$LZMA" ds 17 e $mark\_$TMP $lowercase_tmp_lzma_file 2>&1 );
		}	
	}else{
		print sprintf ("|%s|\n","8M RAM");
		qx("$LZMA" ds 18 e $mark\_$TMP $lowercase_tmp_lzma_file 2>&1 );
	}
	
	$tmp_lzma_size=-s "$lowercase_tmp_lzma_file";
	sysopen(OUTBIN_TMP_LZMA,"$lowercase_tmp_lzma_file",O_BINARY|O_WRONLY|O_APPEND);
	if($tmp_lzma_size&($PAD_LENGTH-1)){
		$temp_pad_length = $PAD_LENGTH - $tmp_lzma_size&($PAD_LENGTH-1);
		#print sprintf ("temp_pad_length %.8x\n",$temp_pad_length);
		while($temp_pad_length--){
			#print OUTBIN_TMP_LZMA '0';
			syswrite(OUTBIN_TMP_LZMA,pack("H1","0"),1);
		}
		#syswrite(OUTBIN_TMP_LZMA,$bufpad,$temp_pad_length);
	}
	close OUTBIN_TMP_LZMA;
	...
	$tmp_size = hex_32_reorder($tmp_lzma_size);
	...
	$tmp_cpz_load_addr = $tmp_cpz_load_addr+$tmp_lzma_size;
	...
	#
	sysopen(OUTBIN_TMP_LZMA,"$lowercase_tmp_lzma_file",O_BINARY|O_RDWR);
	sysopen(OUTBIN_P2,"$IMG_FILENAME\_$PART_2",O_BINARY|O_WRONLY|O_APPEND);
	while(<OUTBIN_TMP_LZMA>){
		print OUTBIN_P2 $_;
	}
...
unlink("$IMG_FILENAME");
close INPUT_STREAM;
$temp_index=index($IMG_GENERAL_INFO,"_TRUNCATE");
if ($temp_index == -1){
file_merge("$IMG_FILENAME\_$PART_1","$IMG_FILENAME\_$PART_2",$IMG_FILENAME);
$compressed_size= -s $IMG_FILENAME;
debug_msg("compressed_size=$compressed_size");
}else{
unlink("$IMG_FILENAME");
rename("$IMG_FILENAME\_$PART_1","$IMG_FILENAME");
$COMPRE_NAME=substr($IMG_FILENAME,0,length($IMG_FILENAME)-4);

rename("$IMG_FILENAME\_$PART_2","$COMPRE_NAME\_$COMPRE");
my $uncompressed_size= -s $IMG_FILENAME;
my $compressed_size_temp= -s "$COMPRE_NAME\_$COMPRE";
$compressed_size = $uncompressed_size + $compressed_size_temp;
}
