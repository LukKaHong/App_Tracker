...
def generate_pkg(target, source, env):
    ...
    src_nm = os.path.join(env['IMAGE_DIR'], env['TARGET_NAME'] + '_Source.zip')
    ...
    env_tmp['aboot_template'] = 'ASR1605_SINGLE_SIM_SMS_04MB'
    env_tmp['aboot_img_info'] = img_info
    env_tmp['aboot_pkg_name'] = pkg_nm
    aboot_template = 'ASR1605_SINGLE_SIM_SMS_04MB'
    ...
    if os.path.exists(env['IMAGE_DIR']):
        ...
        status = subprocess.run([arerease_bin, '-c', PathConfig.ABOOT_PATH, '-g', '-p', 
            'ASR1605_EVB', '-v', aboot_template, '-i', img_info, '--release-pack', src_nm, pkg_nm ], 
