...
#       → FBFMake_CF_V1.6-150.exe 封装 → out/image/system_patch_V{版本}.bin
#       → 计算 SHA-256/大小 → out/image/ota_pkg_info.txt
# 说明：a/system.img 与 b/system.img 为 SDK 基线，脚本不改动；
#       最终升级包名 system_patch_V{APP版本}.bin（版本取自 app_config.h），
...
def main():
    ...
    # 4. 升级包以版本化文件名拷回 out/image，生成平台属性取值信息
    ver = _fw_version()
    if ver:
        final_name = 'system_patch_V%s.bin' % ver
    else:
        print('[ota-pkg] WARNING: 未从 app_config.h 读到 APP_FIRMWARE_VERSION，'
              '使用默认名 system_patch.bin')
        final_name = PATCH_NAME
