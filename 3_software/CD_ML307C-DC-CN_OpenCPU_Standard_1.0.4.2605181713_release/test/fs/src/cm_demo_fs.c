...
/**
 *  CM:FFIND:ALL                        //查找所有文件
 *  CM:FFIND:TXT                        //查找txt类型文件
 *  CM:FFIND:CM                         //查找名称以cm开头的文件
 */

void cm_test_find_file(unsigned char **cmd,int len)
{
    unsigned char operation[20] = {0};
    sprintf((char *)operation, "%s", cmd[2]);
    int ret = 0;

    if (0 == strcmp((const char *)operation, "ALL"))          //HTTP FOTA完整包升级方式，配置FOTA信息
    {
        CM_FS_FILEINFO pFileinfo = {0};
        int32_t number = 0;

        ret = cm_fs_findfirst("*",  &pFileinfo);

        if (ret == 0)
        {
            int32_t i = 0;

            cm_demo_printf("[FFIND] %s\n", pFileinfo.filename);
            number = number + 1;

            for (i = 1;; i++)
            {
                memset((void *)&pFileinfo, 0, sizeof(pFileinfo));
                ret = cm_fs_findnext(&pFileinfo);
                ...
                {
                    //cm_demo_printf("[FFIND] cm_fs_findnext() end\n");
                    break;
                }
            }
        }
        ...
        {
            cm_demo_printf("[FFIND] cm_fs_findfirst() error, return %d\n", ret);
        }
    }
    else if (0 == strcmp((const char *)operation, "TXT"))
    {
        CM_FS_FILEINFO pFileinfo = {0};

        ret = cm_fs_findfirst("*.txt",  &pFileinfo);
    ...
    else if (0 == strcmp((const char *)operation, "CM"))
    {
        CM_FS_FILEINFO pFileinfo = {0};

        ret = cm_fs_findfirst("cm*",  &pFileinfo);
    ...
    {
        cm_demo_printf("[FFIND] Illegal operation\n"); 
    }
}
