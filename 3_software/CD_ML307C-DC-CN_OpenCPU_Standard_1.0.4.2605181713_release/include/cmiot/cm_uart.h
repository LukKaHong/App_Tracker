 ...
 ****************************************************************************/
 ...
 /* 增加UART2，使用PIN38 PIN39，该引脚的默认功能是打印log，若要进行普通串口初始化，需要先用虚拟AT执行 AT+MCFG=log2cat,1 以切换log打印到USB，之后再进行普通串口初始化，该切换配置掉电不保存 */
typedef enum{
    CM_UART_DEV_0,              /*!< 设备0*/
    CM_UART_DEV_1,              /*!< 设备1*/
    CM_UART_DEV_2,              /*!< 设备2*/
...
/** 流控制 */
/* 
 * 开启流控后，当缓存区内数据超过流控上限阈值将拉高RTS(PIN22)，此时若注册了流控事件将上报流控事件
 * 通过读取缓存区数据直到低于流控下限阈值才会重新拉低RTS(PIN22)
 * 若未配置上下限阈值，则上限阈值默认为6K，下限阈值默认为4K，缓存区大小默认为8K
 */
typedef enum{
...
typedef struct{
    ...
    uint32_t is_lpuart;                 /*!< 1:低功耗串口(仅支持115200以内的标准波特率(不支持76800))；0:普通串口*/
    uint32_t rxrb_buf_size;             /*!< 环形缓存区大小，若为0则按默认配置8K；若为0且开了流控，则流控上下限阈值按默认配置6K和4K;自定义值必须配置成2的n次方*/
