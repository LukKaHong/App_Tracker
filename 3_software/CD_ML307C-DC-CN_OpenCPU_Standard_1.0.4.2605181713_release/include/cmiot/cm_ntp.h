 ...
 *          NTP时间同步至系统时间采用cm_rtc_set_date()和cm_rtc_set_time()接口以修改系统时间 \n
 *          NTP服务获取的为时间戳信息，不包括时区信息，回调通知中的时区信息通过cm_rtc_get_timezone()获取 \n
 *          用户未通过cm_rtc_set_timezone()设置时区时，cm_ntp_sync()回调通知中时区不准确，不应使用 \n
