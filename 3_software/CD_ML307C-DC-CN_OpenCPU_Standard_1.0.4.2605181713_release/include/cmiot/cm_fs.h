 ...
 * @{
 * 
 * @warning 开发者请勿尝试修改底层内置系统文件，修改后可能引起不可预知的异常
 */
...
#define CM_FS_FILE_NAME_SIZE (96)
...
typedef struct
{
    ...
    typedef struct {
    char filename[CM_FS_FILE_NAME_SIZE + 1];     // 文件名
} CM_FS_FILEINFO;
 ...
 *   <  0 - 错误
 *
 *  @details 文件系统中无路径/文件夹功能，勿在filename入参中传入路径信息。文件系统最多支持存放100个文件，超过100个文件可能引起不可预期的问题。
 */
 ...
 *   < 0 - 失败
 *
 *  @details 文件系统最多支持存放100个文件，超过100个文件可能引起不可预期的问题。
 */
 ...
 *   < 0 - 失败
 *
 *  @details offset不支持为负数。文件系统最多支持存放100个文件，超过100个文件可能引起不可预期的问题。
 */
 ...
 *   = 0 - 不存在
 *
 *  @details 文件系统中无路径/文件夹功能，勿在path入参中传入路径信息。文件系统最多支持存放100个文件，超过100个文件可能引起不可预期的问题。
 */
 ...
 *  @brief 查找文件
 *
 *  @param [in]  filename 文件名（支持带通配符的文件名）
 ...
 *   < 0 - 失败
 *
 *  @details 仅适用于本平台文件系统，跨平台不能保证兼容。文件系统最多支持存放100个文件，超过100个文件可能引起不可预期的问题。
 */
int32_t cm_fs_findfirst(const char *filename, CM_FS_FILEINFO *fileinfo);
 ...
 *  @details 仅适用于本平台文件系统，跨平台不能保证兼容。文件系统最多支持存放100个文件，超过100个文件可能引起不可预期的问题。
 */
int32_t cm_fs_findnext(CM_FS_FILEINFO *fileinfo);
