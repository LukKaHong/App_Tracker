...
/**********************短信使用注意事项*************************************
 * 
 * 1、为确保短信发送结果的准确性，建议在发送短信前使用cm_virt_at_urc_cb_reg注册"+CMS ERROR:"事件回调
 * 2、结合回调函数返回的结果判断短信发送结果，防止因SIM卡异常等问题导致的接口返回结果与实际发送状态不一致的情况
 * 
 ***************************************************************************/
...
typedef enum
{
    ...
    CM_MSG_MODE_GSM_UCS2,                                   /*编码类型为UCS2*/
}cm_msg_mode_e;
