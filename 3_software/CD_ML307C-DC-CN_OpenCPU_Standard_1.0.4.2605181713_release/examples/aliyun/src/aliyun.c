...
#define printf(...) do{ cm_log_printf(0, "Line[%d]: ", __LINE__); cm_log_printf(0, __VA_ARGS__);}while(0);
...
static uint8_t g_mqtt_handle_is_created = 0;
...
static void __demo_mqtt_recv_thread(void *args)
{
    ...
    while(!g_mqtt_handle_is_created)
    {
        osDelay(10);
    }
...
int cm_opencpu_entry(void *arg)
{
    ...
    mqtt_process_attr.priority = osPriorityNormal1;
    ...
    mqtt_recv_attr.priority = osPriorityNormal1;
