...
static void main_task(void *arg)
{
    ...
    /* ===== 复位自动开机：直接进入 NORMAL 模式并联网，无需长按 5 秒 =====
     * （变更：原"默认关机等待长按开机"改为上电自动开机）
     * 长按 5 秒仍可手动关机 / 关机后再开机 */
    APP_LOGI("auto power on -> NORMAL");
    g_power_on = true;
    bsp_buzzer_beep(2, 100, 100);
    /* RGB 绿->红->蓝 交替闪烁（开机指示） */
    bsp_rgb_set_pattern(BSP_RGB_PATTERN_PLATFORM_CMD, 3);
    app_mode_set(APP_MODE_NORMAL);
    /* 触发联网：等待 CEREG 网络注册成功后连接 MQTT */
    provisioning_and_connect();
...
int cm_opencpu_entry(void *param)
{
    ...
    /* 复位自动开机：main_task 启动后直接进入 NORMAL 模式并联网 */
    APP_LOGI("pet tracker auto power-on at boot");
