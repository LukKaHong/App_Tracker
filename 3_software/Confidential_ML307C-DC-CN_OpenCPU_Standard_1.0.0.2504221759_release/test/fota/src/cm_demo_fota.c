 ...
 *  All rights reserved.
 *  created by cmiot3000 2022/8/29
 ********************************************************/
...
void cm_test_fota(unsigned char **cmd,int len)
{
    ...
    else if (0 == strcmp((const char *)operation, "UPGRADE"))                 //启动升级
    {
        ...
        if (CM_FOTA_TYPE_HTTP == info.fixed_info.fota_mode || CM_FOTA_TYPE_HTTPS == info.fixed_info.fota_mode)
        {
 ...
 *  升级包制作工具在tools\fota_tool，升级包制作以及备份文件预留空间大小请查看tools\fota_tool\升级包制作方法及模组FOTA升级说明.txt；
 *  本示例使用HTTP下载升级包；
 *  CM:OTA:INIT                         //OTA初始化
