...
//设置刷新区域
static void lcd_write_address(unsigned short x_start, unsigned short y_start, unsigned short x_end, unsigned short y_end)
{
...
#else
        lv_color_t *current_draw_buf = NULL;
#if BUFFER_MODE == 1
        current_draw_buf = disp_drv->draw_buf->buf1;
#elif BUFFER_MODE == 2
        // 根据 current_buf 的值选择 buf1 或 buf2
        if(current_buf == 1) {
            current_draw_buf = disp_drv->draw_buf->buf2;
            current_buf = 0; // 切换到另一个缓冲区
        } else {
            current_draw_buf = disp_drv->draw_buf->buf1;
            current_buf = 1; // 切换到另一个缓冲区
        }
#endif
        memcpy(s_lcdbuffer, current_draw_buf, height * width * 2);
#endif
